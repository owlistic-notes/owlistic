...
class NotebooksProvider with ChangeNotifier {
  ...
  void _registerEventHandlers() {
    ...
    _webSocketProvider?.addEventListener(
        'event', 'notebook.updated', _handleNotebookUpdate);
    _webSocketProvider?.addEventListener(
        'event', 'notebook.created', _handleNotebookCreate);
    _webSocketProvider?.addEventListener(
        'event', 'notebook.deleted', _handleNotebookDelete);
    _webSocketProvider?.addEventListener(
        'event', 'note.created', _handleNoteCreate);
    _webSocketProvider?.addEventListener(
        'event', 'note.deleted', _handleNoteDelete);
  ...
  void _handleNotebookUpdate(Map<String, dynamic> message) {
    try {
      ...
      final String? notebookId =
          WebSocketModelExtractor.extractNotebookId(parsedMessage);

      if (notebookId != null) {
        _logger.info(
            'Received notebook.updated event for notebook ID $notebookId');
  ...
  void _handleNotebookCreate(Map<String, dynamic> message) {
    try {
      ...
      if (notebookId != null) {
        _logger.info(
            'Received notebook.created event for notebook ID $notebookId');
        ...
        Future.delayed(Duration(milliseconds: 500), () {
          ...
          ApiService.getNotebook(notebookId).then((newNotebook) {
            ...
            if (_webSocketProvider != null) {
              ...
              _webSocketProvider!
                  .subscribe('notebook:notes', id: newNotebook.id);
            }
  ...
  void _handleNotebookDelete(Map<String, dynamic> message) {
    try {
      ...
      if (notebookId != null) {
        _logger.info(
            'Received notebook.deleted event for notebook ID $notebookId');
  ...
  Future<Notebook> _fetchSingleNotebook(String notebookId) async {
    ...
    try {
      ...
      _logger.info(
          'Fetched notebook ${notebook.id} with ${notebook.notes.length} notes');

      // Check if notebook exists in our list
      if (_notebooksMap.containsKey(notebookId)) {
        _notebooksMap[notebookId] = notebook;
        _logger.info(
            'Updated existing notebook: $notebookId with ${notebook.notes.length} notes');
      } else {
        _notebooksMap[notebook.id] = notebook;
        _logger.info(
            'Added new notebook: $notebookId with ${notebook.notes.length} notes');
  ...
  void _handleNoteCreate(Map<String, dynamic> message) {
    ...
    try {
      ...
      final String? noteId =
          WebSocketModelExtractor.extractNoteId(parsedMessage);
      final String? notebookId =
          WebSocketModelExtractor.extractNotebookId(parsedMessage);

      _logger
          .info('Extracted from event: noteId=$noteId, notebookId=$notebookId');

      // If we have a notebook ID, refresh it
      if (notebookId != null) {
        _logger
            .info('Will refresh notebook $notebookId from note.created event');
    ...
    try {
      ...
      if (notebookId != null && noteId != null) {
        ...
        if (_notebooksMap.containsKey(notebookId)) {
          ...
          final updatedNotes =
              currentNotebook.notes.where((note) => note.id != noteId).toList();
  ...
  Future<void> fetchNotebooks({
    ...
    try {
      ...
      final fetchedNotebooks =
          await ApiService.fetchNotebooks(page: page, pageSize: pageSize);
      ...
      _logger.info(
          'Fetched notebooks (page $page), total: ${_notebooksMap.length}');
    } catch (error) {
  ...
  // Add a method to fetch notes for a specific notebook
  Future<void> fetchNotesForNotebook(String notebookId) async {
    try {
      _logger.info('Fetching notes for notebook: $notebookId');
      final notes = await ApiService.fetchNotesForNotebook(notebookId);

      // Update notebook in map
      if (_notebooksMap.containsKey(notebookId)) {
        final notebook = _notebooksMap[notebookId]!;
        final updatedNotebook = notebook.copyWith(notes: notes);
        _notebooksMap[notebookId] = updatedNotebook;
        notifyListeners();
      }

      _logger.info('Fetched ${notes.length} notes for notebook $notebookId');
    } catch (error) {
      _logger.error('Error fetching notes for notebook $notebookId', error);
    }
  }
  ...
  Future<void> deleteNote(String noteId) async {
    try {
      ...
      // Update notebooks to remove this note
      for (final notebookId in _notebooksMap.keys) {
        final notebook = _notebooksMap[notebookId]!;
        final updatedNotes =
            notebook.notes.where((note) => note.id != noteId).toList();
        if (updatedNotes.length != notebook.notes.length) {
          _notebooksMap[notebookId] = notebook.copyWith(notes: updatedNotes);
        }
      }
